...
import { View, Text, TextInput, TouchableOpacity, KeyboardAvoidingView, Platform, ScrollView, Alert, useWindowDimensions, ImageBackground, Image } from 'react-native';
...
const LoginScreen: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    if (isValid) {
      setIsLoading(true);
      try {
        const success = await login(loginMethod === 'email' ? email : phoneNumber, loginMethod === 'email' ? password : '');
        if (!success) {
          Alert.alert('Login Failed', `Invalid ${loginMethod}${loginMethod === 'email' ? ' or password' : ''}. Please try again.`);
        } else {
          navigation.dispatch(
            CommonActions.reset({
              index: 0,
    ...
    >
      ...
      >
        <ScrollView className="flex-1 px-5">
          ...
          <View className="flex-1 justify-end" style={{ minHeight: height * 0.9 }}>
            <View className="items-center mb-8">
              <Text className="text-3xl font-bold text-white">Brickell Watches</Text>
            </View>

            <View className="bg-white/90 dark:bg-gray-800/90 rounded-xl p-6 shadow-md backdrop-blur-sm mb-5">
              <Text className="text-2xl font-bold text-center text-gray-800 dark:text-white mb-6">Login</Text>

              <View className="flex-row mb-6">
                <TouchableOpacity
                  className={`flex-1 py-3 items-center rounded-lg border mx-1 ${
                    loginMethod === 'email' ? 'bg-blue-50 dark:bg-blue-900 border-primary' : 'border-gray-300 dark:border-gray-600'
                  }`}
                  onPress={() => setLoginMethod('email')}
                >
                  <Text className={`text-sm font-medium ${
                    loginMethod === 'email' ? 'text-primary dark:text-white font-bold' : 'text-gray-500 dark:text-gray-400'
                  }`}>Email</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  className={`flex-1 py-3 items-center rounded-lg border mx-1 ${
                    loginMethod === 'phone' ? 'bg-blue-50 dark:bg-blue-900 border-primary' : 'border-gray-300 dark:border-gray-600'
                  }`}
                  onPress={() => setLoginMethod('phone')}
                >
                  <Text className={`text-sm font-medium ${
                    loginMethod === 'phone' ? 'text-primary dark:text-white font-bold' : 'text-gray-500 dark:text-gray-400'
                  }`}>Phone</Text>
                </TouchableOpacity>
              </View>
              ...
              {loginMethod === 'email' && (
                <View className="mb-4">
                  <View className="flex-row justify-between items-center mb-1">
                    <Text className="text-base text-gray-700 dark:text-gray-300">Password</Text>
                    <TouchableOpacity onPress={() => navigation.navigate('ForgotPassword' as never)}>
                      <Text className="text-sm text-primary dark:text-blue-400">Forgot Password?</Text>
                    </TouchableOpacity>
                  </View>
                  <View className="flex-row items-center border rounded-lg px-3 border-gray-300 dark:border-gray-600">
              ...
              <View className="flex-row justify-center mt-6">
                ...
                <TouchableOpacity onPress={() => navigation.navigate('SignUp' as never)} disabled={isLoading}>
                  <Text className="text-sm text-primary dark:text-blue-400 font-bold">Sign Up</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
