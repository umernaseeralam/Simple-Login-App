...
export const FormField: React.FC<{
  ...
  return (
    <View className="px-4 py-3 border-b border-gray-200">
      ...
      <TextInput
        className={`border border-gray-500 rounded-md px-3 ${multiline ? 'h-32 py-2' : 'h-12'}`}
        value={value}
  ...
  return (
    <View className="flex-row flex-wrap p-4 border-b border-gray-200">
      {options.map((option) => (
        <TouchableOpacity
          key={option}
          className={`px-3 py-1.5 rounded-full border border-gray-500 mr-2 mb-2 ${selectedOption === option ? 'bg-blue-500' : 'bg-transparent'}`}
          onPress={() => onSelect(option)}
        >
          <Text className={selectedOption === option ? 'text-white text-xs' : 'text-gray-500 text-xs'}>
            {option}
          </Text>
        </TouchableOpacity>
      ))}
...
export const styles = {
  ...
  addImageButton: 'h-24 border border-dashed border-gray-500 rounded-lg justify-center items-center',
  ...
  comesWithItem: 'flex-row items-center px-2 py-1 bg-gray-100 rounded-full mr-2 mb-2',
